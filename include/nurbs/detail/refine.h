...
{   
    ...
    for (int col = 0; col < Pw.cols(); ++col) {
        for (int k = 0; k <= a - p; ++K) Qw(k, col) = Pw(k, col);
        for (int k = b - 1; k <= n; ++k) Qw(k + r + 1, col) = Pw(k, col);
    }
    ...
    for (int j = r; j >= 0; --j) 
    {
        ...
        {
            Ubar[k] = U[i];
            for (int col = 0; col < Pw.cols(); ++col)  Qw(k - p -1, col) = Pw(i - p - 1, col);  
            k = k - 1;
            i = i - 1;
            
        }
        for (int col = 0; col < Pw.cols(); ++col)   Qw(k - p - 1, col) = Qw(k - p, col);
        for (int l = 1; l <= p; ++l)
        {
            int ind = k - p + l;
            T alfa = Ubar[k + l] - X[j];
            if (abs(alfa) == 0.0)
                for (int col = 0; col < Pw.cols(); ++col)  Qw(ind - 1, col) = Qw(ind, col);
            else
            {
                alfa = alfa / (Ubar[k + l] - U[i - p + l]);
                for (int col = 0; col < Pw.cols(); ++col)
...
{   
    ...
    Ubar.resize(m+r+2);
    Qw.resize(Qw.rows(), n+r+2);

    int a = findSpan(q, V, X[0]);
    int b = findSpan(q, V, X[r]);
    b = b + 1;
    for (int j = 0; j <= a; ++j) Vbar[j] = V[j];
    for (int j = b + q; j <= m; ++j) Vbar[j + r + 1] = V[j];
    for (int row = 0; row < Pw.rows(); ++row) {
        for (int k = 0; k <= a - q; ++k) Qw(row, k) = Pw(row, k);
        for (int k = b - 1; k <= n; ++k) Qw(row, k + r + 1) = Pw(row, k);
    }
    ...
    for (int j = r; j >= 0; --j) 
    {
        ...
        {
            Vbar[k] = V[i];
            for (int row = 0; row < Pw.rows(); ++row)  Qw(row, k - q -1) = Pw(row, i - q - 1);  
            k = k - 1;
            i = i - 1;
            
        }
        for (int row = 0; row < Pw.rows(); ++row)   Qw(row, k - q - 1) = Qw(row, k - q);
        for (int l = 1; l <= q; ++l)
        {
            int ind = k - q + l;
            T alfa = Vbar[k + l] - X[j];
            if (abs(alfa) == 0.0)
                for (int row = 0; row < Pw.rows(); ++row)  Qw(row, ind - 1) = Qw(row, ind);
            else
            {
...
template<typename T>
vector<int> getinsertKnotVect(vector<T>& U, int n) {
    vector<int> tmp;
